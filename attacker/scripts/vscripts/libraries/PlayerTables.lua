...
function m:start()
	...
	CustomGameEventManager:RegisterListener("PlayerTables_Connected", Dynamic_Wrap(m, "PlayerTables_Connected"))
end
...
function m:PlayerTables_Connected(args)
	...
	for k,v in pairs(m.subscriptions) do
		if v[pid] then
			if player then
				CustomGameEventManager:Send_ServerToPlayer(player, "pt_fu", {name=k, table=m.tables[k]} )
			end
		end
	end
end

function m:CreateTable(tableName, tableContents, pids)
	...
	if self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: player table '" .. tableName .. "' already exists.  Overriding.")
	end
	...
	for k,v in pairs(pids) do
		...
		if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
			...
			DebugPrint("[playertables.lua] Warning: Pid value '" .. pid .. "' is not an integer between [0," .. DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
		end
	end
end

function m:DeleteTable(tableName)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:SetPlayerSubscriptions(tableName, pids)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:AddPlayerSubscription(tableName, pid)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
	...
	if not oldPids[pid] then
		if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
			...
			DebugPrint("[playertables.lua] Warning: Pid value '" .. v .. "' is not an integer between [0," .. DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
		end
	end
end

function m:RemovePlayerSubscription(tableName, pid)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:GetTableValue(tableName, key)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:GetAllTableValues(tableName)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:DeleteTableKey(tableName, key)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:DeleteTableKeys(tableName, keys)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
function m:SetTableValue(tableName, key, value)
	if value == nil then
...
function m:SetTableValues(tableName, changes)
	if not self.tables[tableName] then
		DebugPrint("[playertables.lua] Warning: Table '" .. tableName .. "' does not exist.")
		return
	end
...
if not m.tables then m:start() end
