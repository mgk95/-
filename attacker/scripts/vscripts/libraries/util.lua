		...
		if v ~= 'FDesc' then
			...
			elseif type(value) == "userdata" and not done[value] then
				...
				if t.FDesc and t.FDesc[v] then
					...
					print(string.rep ("\t", indent)..tostring(v)..": "..tostring(value) .. "\t("..type(value)..")")
				end
			end
		end
	end
...
---位运算测试一个value是否包含test值。如果有一个不是数字，则直接返回false
--@param #number value 测试值
--@param #number test 要检查是否存在的值
--@return #boolean 返回value是否包含test位
function BitAndTest(value,test)
	if type(value) == "number" and type(test) == "number" then
		return test == bit32.band(value,test)
	end
	return false;
end
